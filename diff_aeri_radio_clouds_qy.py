...
# main data directory
BUL = os.path.join(home, "Documents", "BLISS", "RamanLidar")
...
# ----------------------------------- #
f_sond = glob(os.path.join(BUL, "*sonde*"))
...
## Find continuous cloudy/cloud-free times index
flist_lidar = sorted(glob('sgprlprofmr2news10mC1.c0.*.000500.nc'))
...
#%% WVMR
diff_wv_a = np.where(np.asarray(a_wv_yy)<0.1)[0][0]
...
plt.tight_layout()
fig.savefig(figpath+'/wvmr_t_p_difference_height.pdf')
#%%
fig = plt.figure(figsize=(5, 8))
ax = plt.gca()
ax.plot(av_wv_xx, z_a_agl,color=colors[0], linestyle="-", linewidth=2,label = 'wvmr AERIvDial')
...
plt.tight_layout()
fig.savefig(figpath+'/wvmr_t_p_difference_log.pdf')
#%%
fig = plt.figure(figsize=(5, 8))
ax = plt.gca()
...
fig.savefig(figpath+'/temp_t_p_difference_height.pdf')
#%%
fig1, ax1 = plt.subplots(nrows=1, ncols=3, sharey=True, figsize=(10, 8))    
#fig1, ax1 = 
ax = ax1[0].twiny()
ax1[0].plot(av_wv_xx, z_a_agl,color=colors[2], linestyle="-", linewidth=2,label = 't AERIvDial')
#ax.scatter(av_wv_xx[diff_wv_av[0]], z_a_agl[diff_wv_av[0]],color=colors[1], marker= '*', label = 'p AERIvDial')
